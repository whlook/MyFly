...
void duty1ms()
{
	if(Flag_data_ready && data_from_slave.data.order[6] == THRO_CRTL)
	{
		...
		{
			front_angle = data_rec_from_slave[5]/10;
			if(front_angle>10)
				front_angle = 10;
		...
		{
			back_angle = data_rec_from_slave[5]/10;
			if(back_angle>10)
				back_angle = 10;
	...
	thro_pwm.pwm1 = (data_from_slave.data.order[7]+(data_from_slave.data.order[8]<<8))*5;
	thro_pwm.pwm2 = (data_from_slave.data.order[7]+(data_from_slave.data.order[8]<<8))*5;
	thro_pwm.pwm3 = (data_from_slave.data.order[7]+(data_from_slave.data.order[8]<<8))*5;
	thro_pwm.pwm4 = (data_from_slave.data.order[7]+(data_from_slave.data.order[8]<<8))*5;
